...
st.set_page_config(
    page_title="Sulfato de Amonio vs Nitroperfecto",
    page_icon="🌱",
    layout="centered"
)
...
inject_css()
render_hero("Sulfato de Amonio vs Nitroperfecto", "Calcula la equivalencia de dosis y compara el costo.")
...
with st.form("form"):
    kg_base = st.number_input("Dosis actual de Sulfato de Amonio (kg/ha)", min_value=0.0, value=1000.0, step=10.0)
    precio_base = st.number_input("Precio de Sulfato de Amonio por tonelada", min_value=0.0, value=8500.0, step=100.0)
    precio_nitro = st.number_input("Precio de Nitroperfecto por tonelada", min_value=0.0, value=17500.0, step=100.0)

    render_locked_values([
        ("Sulfato de Amonio", f"{N_PRODUCTO:g}% N"),
...
<div class="commercial-box">
    <b>Lectura sencilla:</b> El resultado muestra cuánta dosis de Nitroperfecto necesitas y cuánto cuesta comparado contra el producto actual.
        ...
        st.markdown("""
<div class="note-box">
    <b>Mensaje comercial:</b> Útil para explicar al cliente la equivalencia de nitrógeno de forma rápida y sencilla.
</div>
        """, unsafe_allow_html=True)
